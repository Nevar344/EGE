...
# def f(x, y, A): #Создаем функцию
#     return (105 != y + 2*x) or (A>x) or (A>y) #Шаблон просто переписываем выражение
# for A in range(1,1000): #Перебираем А и ставим диапазон поиска
#     if all(f(x,y,A) for x in range(1, 10000) for y in range(10000)): #Говорим что если во всех функция f перебор х и у с этим диапазоном равны 1 правде тогда вывод а
#         print(A)
#         break

#С делителями (ДЕЛ)

# def d(n, m):#Это нужно чтобы не считать деления в функции таким образом уменьшаем шанс на проигрыш
#     return n % m == 0
# def f(x, A):#Переписываем функцию (шаблон)
#     return (d(x, 9) <= (not d(x, 6))) or (x + A >= 100)
# for A in range(1,1000): #Просили НАТУРАЛЬНОЕ значит перебираем не включая 0
#     if all(f(x,A) for x in range(1, 10000)): #Обязательно пишем от 1 до какого то там большого числа
#         print(A)
#         break

#ДЕЛ + Отрезки (когда отрезок извествен)

# def d(n, m):#Это нужно чтобы не считать деления в функции таким образом уменьшаем шанс на проигрыш
#     return n % m == 0
# def f(x, A):
#     return d(x, A) or ((x in list(range(200,301))) <= (not d(x, 77))) #Вот здесь внимательнее т.к нам требует отрезок создаим список в котором диапазон от 200 до 300 по условию и пишем 301 чтобы включить 300
# for A in range(1,1000): #Просили НАТУРАЛЬНОЕ значит перебираем не включая 0
#     if all(f(x,A) for x in range(1,10000)): #Обязательно пишем от 1 до какого то там большого числа
#         print(A)
...
# for A in range(-1000, 1000): #Говорят неотрицательные значит берем вот такой вот диапазон
#     if all(f(x, y, A) for x in range(1000) for y in range(1000)): #Говорят неотриц значит просто 1000 вместо 1, 1000
#         print(A)
#         break

#Треуг(n, m, k)

#Большой шанс выпадения егэ
# def tr(a,b,c): #Шаблон для треугольной функции
#     return a != b != c != 0 and (a+b>c) and (a+c>b) and (b+c>a)
# def f(x, A):
#     return (not ((tr(x, 11, 18) == (not(max(x, 5) > 15))) and tr(x, A, 5)))
# for A in range(1,1000): #Просят натуральное значит от 1
#     if all(f(x, A) for x in range(1,10000)): #Просят натуральное значит от 1
#         print(A)

#Отрезки аналитически!! либо выше смотреть
#ОТрезок но при этом известен параметр А

# A = list(range(3,61)) # сюда вставляем наш отрезок А +1 в конце было 60 стало 61
# B = [x for x in range(2, 177) if 177 % x == 0] #создаем список по условию что Б равно множество натруальных чисел до 177 отличных от 1 и 177 тоесть 1 и 177 не берем в рейндж
# for y in range(10000): #натурал
#     c = [x for x in range(2, y) if y % x == 0] #В условии нам говорят что С это все натруал делители некоторого натурал числа у отличных от единицы и от самого числа у
#     if c:
#         if all(((x in c) <= ((x in A) and (not (x in B)))) for x in range(1,10000)):
#             print(y)

# A = list(range(7,27)) # сюда вставляем наш отрезок А +1 в конце было 26 стало 27
# B = [x for x in range(2, 77) if 77 % x == 0] #создаем список по условию что Б равно множество натруальных чисел до 77 отличных от 1 и 77 тоесть 1 и 77 не берем в рейндж
# for y in range(1, 10000): #натурал
#     c = [x for x in range(2, y) if y % x == 0] #В условии нам говорят что С это все натруал делители некоторого натурал числа у отличных от единицы и от самого числа у
#     if c:
#         if all((x in c)<=((x in A) and (not(x in B))) for x in range(1,10000)):
#             print(y)
