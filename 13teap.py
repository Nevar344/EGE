...
#Сколько в этой сети IP-адресов, для которых в двоео записи-ареса суматое количество единиц в левых двух байтахе менее суммарного
#количества единиц в правых двух байтах? В ответе укажите только число.
from ipaddress import *

k = 0
for ip in ip_network('23.140.159.160/255.255.252.0', 0):
    b = ip.packed
    left = bin(b[0]).count('1') + bin(b[1]).count('1')
    right = bin(b[2]).count('1') + bin(b[3]).count('1')
    if left >= right:
        k += 1
print(k)
